...
const getUsersByEyeColor = (arr, color) => {
  return arr.filter(x=>x.eyeColor===color); 
};
...
const getUserByEmail = (arr, email) => {
  return arr.find(x=>x.email===email);
};
...
const getUsersWithAge = (arr, min, max) => {
  const minCutArr = arr.filter(x=>x.age>min);
  return minCutArr.filter(x=>x.age<max);
};
...
const getTotalBalance = arr => {
  return arr.reduce((acc, curr)=>acc+curr.balance, 0);
};
